...
struct StationParameters {
	...
	StationParameters(double fmin, double fmax, double dmin, double dmax, int half_beam = 400) : flare_min(fmin), flare_max(fmax), deadrise_min(dmin), deadrise_max(dmax), half_beam(half_beam) {}

	StationParameters() : flare_min(0.0), flare_max(90.0), deadrise_min(0.0), deadrise_max(90.0), z_coord(0), half_beam(400) {}
